...
const EstablishmentForm = () => {
	...
	const handleAddEstablishment = async data => {
		...
		formData.bookingIncludes = await processFile(data.bookingIncludes[0]);
		formData.popularFacilityIcons = await processFile(data.popularFacilityIcons[0]);
		formData.facilityIcons = await processFile(data.facilityIcons[0]);
		formData.stars = await processFile(data.stars[0]);
